"""
inference.py ― SubwayCooling PPO 위치 확장 버전
────────────────────────────────────────────────────────
...
from subway_env_v2 import SubwayCoolingEnv   # 위치 변수 포함된 v2
...
def run_inference(state: np.ndarray, deterministic: bool = True):
    """
    state shape=(5,)  [time, passengers, comfort, x_pos, y_pos]
    ...
    print(f"[t={state[0]:02.0f}, pax={state[1]:3.0f}, "
          f"c={state[2]:+.2f}, x={state[3]:.2f}, y={state[4]:.2f}] "
          f"→ temp {real_temp:4.1f} ℃(lv {tlv:2}), fan {real_fan}({flv})")
    return tlv, flv
...
if __name__ == "__main__":
    ...
    for _ in range(args.num):
        t  = rng.integers(0, 24)             # 시간
        p  = rng.integers(10, 101)           # 승객 수
        c  = rng.uniform(-1.0, 1.0)          # comfort
        x  = rng.uniform(0.0, 1.0)           # 위치 x
        y  = rng.uniform(0.0, 1.0)           # 위치 y
        s  = np.array([t, p, c, x, y], dtype=np.float32)
